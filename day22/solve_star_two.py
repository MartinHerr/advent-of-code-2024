...
def next_secret(number, secret=16777216):
    number = (number ^ (number * 64)) % secret
    number = (number ^ (number // 32)) % secret
    number = (number ^ (number * 2048)) % secret
    return number

def max_sales(numbers, iterations=2000):
    ret = 0
    total = {}
